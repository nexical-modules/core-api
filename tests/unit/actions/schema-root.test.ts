...
vi.mock('../../../src/actions/../services/core-doc-service');

describe('SchemaRootAction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should be defined', () => {
    expect(SchemaRootAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const coreDocServiceMock = vi.mocked(CoreDocService);
    commonServiceMethods.forEach((m) => {
      if ((coreDocServiceMock as unknown as Record<string, vi.Mock>)[m])
        (coreDocServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await SchemaRootAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call CoreDocService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when CoreDocService fails', async () => {
    const serviceMock = vi.mocked(CoreDocService);
    commonServiceMethods.forEach((m) => {
